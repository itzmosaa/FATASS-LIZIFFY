...
run(function()
    ...
    local function createRangeCircle()
        local suc, err = pcall(function()
            if (not shared.CheatEngineMode) then
                ...
                if Killaurabetter.Enabled then
        ...
        if (not suc) then
            pcall(function()
                ...
                notif("Killaurabetter - Range Visualiser Circle", "There was an error creating the circle. Disabling...", 2)
            end)
        end
    end
	...
	local function startAutoFireballLoop()
        ...
        autoFireballLoop = task.spawn(function()
            while AutoFireball and AutoFireball.Enabled do
                if entitylib.isAlive then
                    local pos = entitylib.character.RootPart.Position
                    local closest = store.KillaurabetterTarget
                    if closest and closest.RootPart and Killaurabetter.Enabled then
    ...
    local function doOldFastHits()
        if not store.KillaurabetterTarget then return end
    ...
    local function doFastHits()
        ...
        if FastHitsHitsRequiredToggle and FastHitsHitsRequiredToggle.Enabled then
            ...
            if fastHitsTrackedEntity and fastHitsTrackedEntity ~= store.KillaurabetterTarget then
                fastHitsActivationReady = false
                return
            end
        end

        local ent = store.KillaurabetterTarget
        if not ent or not ent.RootPart then return end
    ...
    local function startAutoShootLoop()
        ...
        autoShootLoop = task.spawn(function()
            while Killaurabetter.Enabled and FastHits.Enabled do
                doFastHits()
                task.wait(0.05)  
            end
            autoShootLoop = nil
    ...
    KillaurabetterBetter = vape.Categories.Blatant:CreateModule({
        Name = 'KillaurabetterBetter',
        Function = function(callback)
            if callback then 
				local attacked = {}   
                ...
                if Mouse and LegitAura and Mouse.Enabled and LegitAura.Enabled then
                    ...
                    notif("Killaurabetter", "yo u cant have require mouse down AND swing only both on at da same time turned both off 4 u", 5)
                end
                ...
                if Animation.Enabled and not (identifyexecutor and table.find({'Argon', 'Delta'}, ({identifyexecutor()})[1])) then
                    ...
                    task.spawn(function()
                        ...
                        repeat
                            if Attacking then
                                ...
                                for _, v in anims[AnimationMode.Value] do
                                    ...
                                    AnimTween.Completed:Wait()
                                    first = false
                                    if (not Killaurabetter.Enabled) or (not Attacking) then break end
                                end
                            elseif started then
                                started = false
                                AnimTween = tweenService:Create(gameCamera.Viewmodel.RightHand.RightWrist, TweenInfo.new(AnimationTween.Enabled and 0.001 or 0.3, Enum.EasingStyle.Exponential), {
                        ...
                        until (not Killaurabetter.Enabled) or (not Animation.Enabled)
                    end)
                end
                ...
                repeat
					if AttackCheck and AttackCheck.Enabled then
                        ...
                        if triggered then
                            Attacking = false
                            store.KillaurabetterTarget = nil
                            task.wait(0.3)
                            continue
                        end
                    end
                    ...
                    if vapeTargetInfo and vapeTargetInfo.Targets then
                        vapeTargetInfo.Targets.Killaurabetter = nil
                    end
                        ...
                        if shouldSwing then
                            ...
                            if hasValidAttackTargets then
                                for _, v in attackPlrs do
                                    ...
                                    if vapeTargetInfo and vapeTargetInfo.Targets then
                                        local _vapeKillaurabetterInfo = {
                                            Humanoid = {Health = 0, MaxHealth = 0},
                                            Player = nil
                                        }
                                        _vapeKillaurabetterInfo.Humanoid.Health = v.Health
                                        _vapeKillaurabetterInfo.Humanoid.MaxHealth = v.MaxHealth
                                        _vapeKillaurabetterInfo.Player = v.Player
                                        vapeTargetInfo.Targets.Killaurabetter = _vapeKillaurabetterInfo
                                    end

                                    if not Attacking then
                                        Attacking = true
                                        store.KillaurabetterTarget = v
                                        if not isClaw then
                    ...
                    task.wait(1 / UpdateRate.Value)
                until not Killaurabetter.Enabled
            else
    ...
    pcall(function()
        local PSI = Killaurabetter:CreateToggle({
    ...
    Targets = Killaurabetter:CreateTargets({
        Players = true,
        NPCs = true
    })
    
    TargetPriority = Killaurabetter:CreateDropdown({
    ...
    SwingRange = Killaurabetter:CreateSlider({
        Name = 'Swing range',
        Min = 1,
        Max = 40, 
        Default = 22, 
    ...
    AttackRange = Killaurabetter:CreateSlider({
        Name = 'Attack range',
        Min = 1,
        Max = 22,
        Default = 22, 
    ...
    RangeCircle = Killaurabetter:CreateToggle({
    ...
    AngleSlider = Killaurabetter:CreateSlider({
        Name = 'Max angle',
        Min = 1,
        Max = 360,
        Default = 360
    })
    UpdateRate = Killaurabetter:CreateSlider({
        Name = 'Update rate',
        Min = 1,
        Max = 120,
        Default = 60,
    ...
    MaxTargets = Killaurabetter:CreateSlider({
        Name = 'Max targets',
        Min = 1,
        Max = 5,
        Default = 5
    })
    Sort = Killaurabetter:CreateDropdown({
        Name = 'Target Mode',
        List = methods
    })
    Mouse = Killaurabetter:CreateToggle({
        Name = 'Require mouse down',
        Function = function(callback)
            if callback and LegitAura and LegitAura.Enabled then
                Mouse:Toggle(false)
                LegitAura:Toggle(false)
                notif("Killaurabetter", "yo u cant have require mouse down AND swing only on at da same time turned both off 4 u ", 5)
            end
        end
    })
    Swing = Killaurabetter:CreateToggle({Name = 'No Swing'})
    GUI = Killaurabetter:CreateToggle({Name = 'GUI check'})
    SwingTime = Killaurabetter:CreateToggle({
        Name = 'Custom Swing Time',
        Function = function(callback)
            SwingTimeSlider.Object.Visible = callback
        end
    })
    SwingTimeSlider = Killaurabetter:CreateSlider({
        Name = 'Swing Time',
        Min = 0,
        Max = 1,
        Default = 0.42,
    ...
    ContinueSwinging = Killaurabetter:CreateToggle({
    ...
    ContinueSwingTime = Killaurabetter:CreateSlider({
        Name = 'Swing Duration',
        Min = 0,  
        Max = 5,  
        Default = 1,
    ...
    CustomHitReg = Killaurabetter:CreateToggle({
        Name = 'Custom Hit Reg',
    ...
    CustomHitRegSlider = Killaurabetter:CreateSlider({
    ...
    SyncHits = Killaurabetter:CreateToggle({
        Name = 'Sync Hits',
        Tooltip = 'Waits for sword animation before attacking'
    })
    Killaurabetter:CreateToggle({
        Name = 'Show target',
        Function = function(callback)
            BoxSwingColor.Object.Visible = callback
            BoxAttackColor.Object.Visible = callback
    ...
    BoxSwingColor = Killaurabetter:CreateColorSlider({
        Name = 'Target Color',
        Darker = true,
        DefaultHue = 0.6,
        DefaultOpacity = 0.5,
        Visible = false,
        Function = function(hue, sat, val)
            if Killaurabetter.Enabled and RangeCirclePart ~= nil then
                RangeCirclePart.Color = Color3.fromHSV(hue, sat, val)
            end
        end
    })
    BoxAttackColor = Killaurabetter:CreateColorSlider({
        Name = 'Attack Color',
        Darker = true,
        DefaultOpacity = 0.5,
        Visible = false
        ...
        Function = function(callback)
            ...
            if callback then
                for i = 1, 10 do
                    ...
                    part.Transparency = 1
                    part.CanQuery = false
                    part.Parent = Killaurabetter.Enabled and gameCamera or nil
                    local particles = Instance.new('ParticleEmitter')
                    particles.Brightness = 1.5
                    particles.Size = NumberSequence.new(ParticleSize.Value)
                    particles.Shape = Enum.ParticleEmitterShape.Sphere
    ...
    ParticleTexture = Killaurabetter:CreateTextBox({
        Name = 'Texture',
        Default = 'rbxassetid://14736249347',
        Function = function()
            for _, v in Particles do
    ...
    ParticleColor1 = Killaurabetter:CreateColorSlider({
        Name = 'Color Begin',
        Function = function(hue, sat, val)
            for _, v in Particles do
                v.ParticleEmitter.Color = ColorSequence.new({
    ...
    ParticleColor2 = Killaurabetter:CreateColorSlider({
        Name = 'Color End',
        Function = function(hue, sat, val)
            for _, v in Particles do
                v.ParticleEmitter.Color = ColorSequence.new({
    ...
    ParticleSize = Killaurabetter:CreateSlider({
        Name = 'Size',
        Min = 0,
        Max = 1,
        Default = 0.2,
    ...
    Face = Killaurabetter:CreateToggle({
        Name = 'Face target',
        Function = function(callback)
    ...
    FaceSpeed = Killaurabetter:CreateSlider({
        Name = 'Face Speed',
        Min = 1,
        Max = 100,
        Default = 15,
    ...
    Animation = Killaurabetter:CreateToggle({
        Name = 'Custom Animation',
        Function = function(callback)
            AnimationMode.Object.Visible = callback
            AnimationTween.Object.Visible = callback
    ...
    AnimationMode = Killaurabetter:CreateDropdown({
        Name = 'Animation Mode',
        List = animnames,
        Darker = true,
        Visible = false
    })
    AnimationSpeed = Killaurabetter:CreateSlider({
        Name = 'Animation Speed',
        Min = 0,
        Max = 2,
        Default = 1,
    ...
    AnimationTween = Killaurabetter:CreateToggle({
        Name = 'No Tween',
        Darker = true,
        Visible = false
    })
    Limit = Killaurabetter:CreateToggle({
        Name = 'Limit to items',
        Function = function(callback)
            if inputService.TouchEnabled and Killaurabetter.Enabled then
                pcall(function()
                    lplr.PlayerGui.MobileUI['2'].Visible = callback
                end)
            end
    ...
    LegitAura = Killaurabetter:CreateToggle({
        Name = 'Swing only',
        Tooltip = 'Only attacks while swinging manually',
        Function = function(callback)
            if callback and Mouse and Mouse.Enabled then
                LegitAura:Toggle(false)
                Mouse:Toggle(false)
                notif("Killaurabetter", "yo u cant have swing only AND require mouse down on at da same time lol turned both off 4 u ", 5)
            end
        end
    })
    AirHit = Killaurabetter:CreateToggle({
        Name = 'Air Hits',
        Default = true,
        ...
        Function = function(callback)
            ...
            if Killaurabetter.Enabled and callback and AirHitsChance and AirHitsChance.Object then
                AirHitsChance.Object.Visible = true
            end
        end
    })
    AirHitsChance = Killaurabetter:CreateSlider({
    ...
    AttackCheck = Killaurabetter:CreateToggle({
        Name = 'Attack Check',
        Tooltip = 'Stops Killaurabetter when a kit ability is detected (Sophia, etc) or when asleep',
        Function = function(callback)
        end,
        Default = false
    })

	FastHits = Killaurabetter:CreateToggle({
        Name = 'Fast Hits',
    ...
    FastHitsMode = Killaurabetter:CreateDropdown({
        Name = 'Fast Hits Mode',
    ...
    LegitSwitch = Killaurabetter:CreateToggle({
        Name = 'Legit Switch',
        Default = false,
        Darker = true,
        Visible = false,
    ...
    OldShootInterval = Killaurabetter:CreateSlider({
        Name = 'Shoot Interval',
        Min = 0.1,
        Max = 3,
        Default = 0.5,
    ...
    OldSwitchDelay = Killaurabetter:CreateSlider({
        Name = 'Switch Delay',
        Min = 0,
        Max = 0.2,
        Default = 0.05,
    ...
    OldWaitDelay = Killaurabetter:CreateSlider({
        Name = 'Wait Delay',
        Min = 0,
        Max = 1,
        Default = 0,
    ...
    OldFirstPersonCheck = Killaurabetter:CreateToggle({
	...
	AutoFireball = Killaurabetter:CreateToggle({
        Name = 'Auto Fireball',
        Default = false,
        Visible = false,
    ...
    FireRate = Killaurabetter:CreateSlider({
        Name = 'Fire rate',
        Suffix = 's',
        Min = 0,
        Max = 2,
